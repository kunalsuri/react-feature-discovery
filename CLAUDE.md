<!-- Companion to AGENTS.md — update both together. -->
...
- `.agent/workflows/` — pre-existing agent workflow notes (unrelated to this `.claude/`/`.agents/` pairing — leave as-is)
...
- No CLAUDE Code-specific subagents, slash commands, or skills exist yet beyond `.claude/skills/multi-agent-docs/` — add `commands/`, `agents/`, or further `skills/` subfolders under `.claude/` only when there's a real one to add.
